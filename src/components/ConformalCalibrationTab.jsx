  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-600 mt-1 max-w-2xl leading-relaxed">
              Distribution-free, finite-sample-valid coverage guarantee: <span className="font-mono font-bold text-indigo-700">P(Y_(n+1) ∈ Ĉ(X_(n+1))) ≥ 1 - α</span>, guaranteeing that risk-managed prediction sets never systematically under-cover during market distress.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-right font-mono text-xs shadow-sm">
              ...
              <div className="text-emerald-600 font-bold text-base">{CONFORMAL_CALIBRATION.ece}</div>
            </div>
            <div className="px-4 py-2 rounded-xl bg-white border border-slate-200 text-right font-mono text-xs shadow-sm">
              <div className="text-[10px] text-slate-400 font-semibold">RPS SKILL SCORE</div>
              <div className="text-indigo-600 font-bold text-base">+{CONFORMAL_CALIBRATION.rpsSkillScore}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="quant-card p-5 rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-900">Interactive Target Coverage Level (1 - α)</span>
          </div>
          <div className="flex items-center gap-3 font-mono text-xs">
            <span className="text-slate-500">α = {alpha}</span>
            <span className="px-3 py-1 rounded-lg bg-indigo-50 text-indigo-700 font-bold text-sm border border-indigo-200">
              {targetCoverage}% Coverage
            </span>
            <span className="text-slate-500">Mean Set Size: <strong className="text-slate-900">{estimatedMeanSetSize}</strong></span>
          </div>
        </div>
        <input
          type="range"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 quant-card p-5 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
              <Target className="w-4 h-4 text-indigo-600" />
              <span>Mondrian Class-Conditional Coverage</span>
            </h3>
            <span className="text-xs font-mono text-slate-500 font-medium">Target: 90.0%</span>
          </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {CONFORMAL_CALIBRATION.mondrianClasses.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="py-2.5 font-semibold text-slate-800">{item.regime}</td>
                    <td className="py-2.5 text-slate-500">{item.target}</td>
                    <td className="py-2.5 text-emerald-600 font-bold">{item.observed}</td>
                    <td className="py-2.5 text-slate-700">{item.meanSetSize}</td>
        ...
        <div className="lg:col-span-6 quant-card p-5 rounded-2xl border border-slate-200 bg-white shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-mono text-emerald-600 font-bold">ECE: 0.0156</span>
          </div>
          <p className="text-xs text-slate-600 leading-relaxed">
            ...
            {reliabilityBins.map((bin, idx) => (
              <div key={idx} className="space-y-1 text-[11px] font-mono">
                <div className="flex justify-between text-slate-500">
                  ...
                  <span>
                    Forecast: {bin.forecast.toFixed(2)} | Observed: <strong className="text-slate-800">{bin.observed.toFixed(3)}</strong>
                  </span>
                </div>
          ...
          <div className="pt-3 border-t border-slate-100 grid grid-cols-3 gap-2 text-center text-xs font-mono">
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200">
              ...
              <div className="text-slate-900 font-bold">{CONFORMAL_CALIBRATION.brierDecomposition.reliability}</div>
            </div>
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200">
              <div className="text-[10px] text-slate-400 font-semibold">BRIER RESOLUTION</div>
              <div className="text-slate-900 font-bold">{CONFORMAL_CALIBRATION.brierDecomposition.resolution}</div>
            </div>
            <div className="p-2.5 rounded-lg bg-slate-50 border border-slate-200">
              <div className="text-[10px] text-slate-400 font-semibold">BRIER SCORE</div>
              <div className="text-emerald-600 font-bold">{CONFORMAL_CALIBRATION.brierScore}</div>
            </div>
          </div>
        </div>
      </div>
